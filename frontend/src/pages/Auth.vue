    ...
    methods: {
      login: function() {
        if (!this.$validateHex(this.publicKey)) {
          return this.$notify('error', 'Invalid public key has been passed')
        }

        if (!this.$validateHex(this.secretKey, 64)) {
          return this.$notify('error', 'Invalid secret key has been passed')
        }
      ...
      register: function() {
        ...
        if (!this.name) {
          return this.$notify('error', 'Empty name has been passed')
        }
